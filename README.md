# VAD-Cut

![VAD-Cut 截图](build/app.png)

课程视频**掐头去尾**剪辑工具。基于 Silero VAD 自动定位第一句话和最后一句话的位置，批量裁掉录屏前后的静默片段。

## 功能特性

| 功能 | 说明 |
|------|------|
| 自动剪辑 | Silero VAD 精确定位语音边界，自动去头尾静默 |
| 批量处理 | 扫描整个文件夹，最多 4 路并发 |
| 智能编码 | 自动探测 NVIDIA NVENC / AMD AMF / Intel QSV，无 GPU 时降级 CPU (libx264) |
| 画质优化 | 缩放 720p / 24fps，降噪 + 色彩均衡，输出 MP4 1500kbps |
| 字幕生成 | （可选）sherpa-onnx-node（Native ONNX，运行时固定走 vendored Windows x64 runtime）+ SenseVoice / Paraformer，输出 `.srt` |

## 支持格式

**输入：** `.mp4` `.mkv` `.avi` `.mov` `.mts` `.m2ts` `.ts` `.flv` `.wmv` `.webm` `.mpg` `.mpeg` `.m4v`

**输出：** `.mp4`

## 快速开始

```bash
# 1. 安装依赖
npm install

# 2. 下载 VAD 模型（silero_vad.onnx，约 630KB）
npm run setup

# 3. 启动
npm start
```

### 字幕功能（可选）

首次启动后会自动检测字幕引擎；若未安装，会弹出「引擎下载」窗口，下载完成后即可使用字幕提取。

- 引擎模型来源（ModelScope）：
  `Mr7Cat/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2025-09-09`
- 首次下载体积约 226MB（`model.int8.onnx` + `tokens.txt`）
- 下载后的字幕模型存放在：
  `%USERPROFILE%\\.vadCut\\model\\sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2025-09-09`
- 使用 NSIS 安装包卸载时，可勾选删除已下载的字幕模型

## 使用方法

1. 点击中央区域，选择包含视频的文件夹
2. 按需勾选「生成字幕」
3. 点击「开始剪辑」
4. 剪辑结果保存在原文件夹下的 `剪辑/` 子目录

```
课程录屏/
├── 01.mts
├── 02.mts
└── 剪辑/
    ├── 01.mp4
    ├── 01.srt        ← 勾选字幕时生成
    ├── 02.mp4
    └── 02.srt
```

## 打包发布

```bash
npm run build
```

输出到 `dist/`（Windows NSIS 安装包）。

## 项目结构

```
electron/
  main.js            Electron 主进程
  preload.js         预加载脚本
renderer/            前端页面（HTML / CSS / JS）
src/
  ffmpegUtils.js     ffmpeg 封装（编码器探测、剪辑、音频提取）
  vad.js             sherpa-onnx 原生 VAD（Silero）
  asr.js             sherpa-onnx-node ASR（字幕生成，Native）
  processor.js       核心处理逻辑（文件扫描、并发调度）
scripts/
  download-model.js  下载 VAD 模型
  gen-icon.js        生成桌面图标
models/              模型文件（Git LFS，不含在源码中）
```

## 技术栈

- [Electron](https://www.electronjs.org/) — 桌面应用框架
- [fluent-ffmpeg](https://github.com/fluent-ffmpeg/node-fluent-ffmpeg) + [ffmpeg-static](https://github.com/eugeneware/ffmpeg-static) — 视频处理
- [sherpa-onnx-node](https://github.com/k2-fsa/sherpa-onnx) — Native ONNX VAD / ASR（JS 封装层）
- `vendor/sherpa-onnx-win-x64` — 唯一 Windows x64 Native ONNX runtime（二进制来源）
